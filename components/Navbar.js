...
const Navbar = () => {
  ...
  return (
    <nav>
      ...
      <Link href="/"><a className="brand-logo center">WuWuMelon</a></Link>
      <ul id="nav-mobile" className="left hide-on-med-and-down">
        {user ? <li><button onClick={logout}><a >Logout</a></button></li>  : <li><button onClick={login}><a>Sign In</a></button></li>}

        {/* {user ? <li><Link href="/user/upload"><a>Logout</a></Link></li>  : <li><Link href="/authentication/Signup"><a>Sign In</a></Link></li>} */}
        {user ? <li><Link href="/user/profile"><a>Account</a></Link></li> : null}
      </ul>
    </div>
